...
def handle_dialog(req, res):
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = f'{jiv[sessions[user_id]]} можно найти на Яндекс.Маркете!'
        if sessions[user_id] == 2:
    ...
    res['response']['text'] = \
        f"Все говорят '{req['request']['original_utterance']}', а ты купи {jiv[sessions[user_id]]}!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        suggests.append({
            "title": "Ладно",
            "url": f"https://market.yandex.ru/search?text={jiv[sessions[user_id]]}",
            "hide": True
        })
